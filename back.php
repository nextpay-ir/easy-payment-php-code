...
$result_trans=mysql_query("SELECT `amount` FROM `order` WHERE `ref` = '$trans_id'");
if(mysql_num_rows($result_trans)>0){
	list($price)=mysql_fetch_array($result_trans);
}else{
...
try {
      ...
      if( $result < 0 ) {
	      ...
	      mysql_query("update `order` set `status`='y',`ref`='$trans_id' where `id`='$res_num' limit 1");
