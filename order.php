...
<?php
	...
	*/
	...
	try {
		  ...
		  mysql_query("INSERT INTO `order` (`email` ,`tel` ,`name`,`comment` ,`amount` ,`status` ,`ref` ,`date`) VALUES ('{$_POST['TxtEmail']}', '{$_POST['TxtMobile']}','{$_POST['TxtName']}','{$_POST['TxtTitle']}','{$_POST['TxtPrice']}', 'n', '{$result->trans_id}','".time()."');");
		  echo mysql_error();
		  $id=mysql_insert_id();
		  if($id<=0){echo 'خطا در ایجاد سفارش .';exit;}
		  $nextpay->send($result->trans_id);
	      } else {
		  $data['message'] = '<font color="red">در ارتباط با درگاه dargahbank.ir مشکلی به وجود آمده است. لطفا مطمئن شوید شناسه درگاه خود را به درستی در قسمت مدیریت وارد کرده اید.</font> شماره خطا: '.$result.'<br />';
